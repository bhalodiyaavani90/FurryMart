...
// 2. DYNAMIC CONTENT FETCH - Get brands for Accessories category
$brand_res = mysqli_query($conn, "SELECT brand_name FROM brands WHERE category = 'Bowls' AND status = 'active' ORDER BY brand_name ASC");

// 3. MASTER SYNC QUERY (Products + Variants Joined) - Filter for Bowls & Feeders 
$sql = "SELECT p.*, v.price, v.mrp, v.weight_size, v.stock_qty, v.id as variant_id 
        ...
        WHERE p.is_available = 1 AND p.sub_cat_id = 10 ";
...
<style>
    ...
    .hero-glimmer { position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: linear-gradient(rgba(15,28,63,0.7), rgba(15,28,63,0.7)), url('uploads/productpageHero/dogBowlshero.jpg'); background-size: cover; background-position: center; animation: slowReflect 20s infinite alternate; z-index: -1; }
...
<div class="hero-vault animate__animated animate__fadeIn">
    ...
    <div class="animate__animated animate__zoomIn" style="text-align: center;">
        <h1>Bowls & Feeders Hub</h1>
        <p style="font-weight: 800; text-transform: uppercase; letter-spacing: 4px; color: rgba(255,255,255,0.95);">Feeding Solutions & Accessories</p>
    </div>
</div>
...
<div class="catalog-viewport">
    <aside class="sidebar-filters">
        ...
        <div class="filter-group">
            ...
            <a href="dog_Bowles&Feeders.php" class="st-link <?php if(!$current_stage) echo 'active'; ?>">All Stages</a>
        ...
        <div class="filter-group">
            ...
            <?php while($b = mysqli_fetch_assoc($brand_res)): ?>
                <a href="?brand=<?php echo $b['brand_name']; ?>" class="st-link <?php if($brand_filter == $b['brand_name']) echo 'active'; ?>"><?php echo $b['brand_name']; ?></a>
            <?php endwhile; ?>
        </div>
    ...
    <main class="product-grid">
        ...
        <div class="product-card animate__animated animate__fadeInUp">
            ...
            <button class="btn-action" onclick="addToCart(<?php echo $p['variant_id']; ?>, '<?php echo addslashes($p['name']); ?>', '<?php echo $p['base_image']; ?>', <?php echo $p['price']; ?>, '<?php echo $p['weight_size']; ?>')">ADD TO BAG</button>
        ...
        <div class="modal-console">
            ...
            <div class="console-scroll-zone">
                ...
                <div class="dash-grid">
                    ...
                    <div class="dash-item">
                        <small class="dash-label">Net Weight/Size</small>
...
function launchQuickView(p) {
    ...
    // Hide diet badge for bowls & feeders
    document.getElementById('qv-diet-badge').style.display = 'none';
...
function addToCart(variantId, name, image, price, weight) {
    ...
    .then(data => {
        ...
        if(typeof updateHeaderCounts === 'function') updateHeaderCounts(); // Update cart count in header
