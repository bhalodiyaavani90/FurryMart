...
// 1. CAPTURE ALL 4 INTELLIGENCE FILTERS
...
// 2. DYNAMIC CONTENT FETCH - Get available brands for this category
$brand_res = mysqli_query($conn, "SELECT DISTINCT brand_name FROM products WHERE is_available = 1 AND main_cat_id = 4 AND sub_cat_id = 3 ORDER BY brand_name ASC");
...
$sql = "SELECT p.*, v.price, v.mrp, v.weight_size, v.stock_qty, v.id as variant_id 
        ...
        WHERE p.is_available = 1 AND p.main_cat_id = 4 AND p.sub_cat_id = 3";
...
<style>
    ...
    .hero-glimmer { position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: linear-gradient(rgba(15,28,63,0.7), rgba(15,28,63,0.7)), url('uploads/productpageHero/dogfoodhero.jpg'); background-size: cover; background-position: center; animation: slowReflect 20s infinite alternate; z-index: -1; }
    ...
    /* --- DIET TOGGLE SWITCH (VEG/NON-VEG) --- */
    .diet-toggle-container {
        display: flex;
    ...
    .diet-label.active { color: var(--navy); }
    .diet-label.inactive { color: var(--gray); opacity: 0.5; }
    ...
    .diet-icon.veg-icon { border-color: var(--veg); }
    .diet-icon.veg-icon::after { content: ''; width: 10px; height: 10px; background: var(--veg); border-radius: 50%; }
    .diet-icon.nonveg-icon { border-color: var(--nonveg); }
    .diet-icon.nonveg-icon::after { content: ''; width: 10px; height: 10px; background: var(--nonveg); border-radius: 50%; }
    ...
    .toggle-switch.veg-active { background: var(--veg); }
    .toggle-switch.nonveg-active { background: var(--nonveg); }
    ...
    .toggle-switch.nonveg-active .toggle-slider { transform: translateX(30px); }
    ...
    .console-scroll-zone { padding: 30px 50px; overflow-y: auto; flex-grow: 1; scroll-behavior: smooth; max-height: calc(99vh - 250px); }
...
<div class="hero-vault animate__animated animate__fadeIn">
    ...
    <div class="animate__animated animate__zoomIn" style="text-align: center;">
        <h1>Dog Food Hub</h1>
        <p style="font-weight: 800; text-transform: uppercase; letter-spacing: 4px; color: rgba(255,255,255,0.95);">A-to-Z Optimized Nutrition Protocols</p>
    </div>
</div>

<!-- DIET TOGGLE SWITCH -->
<div class="diet-toggle-container animate__animated animate__fadeInDown">
    <div class="diet-label <?php echo (!$diet_filter || $diet_filter=='Veg') ? 'active' : 'inactive'; ?>">
        <span class="diet-icon veg-icon"></span>
        <span>VEG</span>
    </div>
    
    <div class="toggle-switch <?php echo ($diet_filter=='Non-Veg') ? 'nonveg-active' : 'veg-active'; ?>" 
         onclick="toggleDiet()">
        <div class="toggle-slider"></div>
    </div>
    
    <div class="diet-label <?php echo ($diet_filter=='Non-Veg') ? 'active' : 'inactive'; ?>">
        <span>NON-VEG</span>
        <span class="diet-icon nonveg-icon"></span>
    </div>
</div>
...
<div class="catalog-viewport">
    <aside class="sidebar-filters">
        ...
        <div class="filter-group">
            ...
            <a href="dog_food.php" class="st-link <?php if(!$current_stage) echo 'active'; ?>">All Stages</a>
...
/**
 * DIET TOGGLE FUNCTION - VEG/NON-VEG SWITCH
 */
function toggleDiet() {
    const currentParams = new URLSearchParams(window.location.search);
    const currentDiet = currentParams.get('diet');
    
    // Toggle between Veg and Non-Veg
    if (currentDiet === 'Non-Veg' || !currentDiet) {
        currentParams.set('diet', 'Veg');
    } else {
        currentParams.set('diet', 'Non-Veg');
    }
    
    // Redirect with new diet parameter
