...
// 2. MASTER SYNC QUERY (Products + Variants Joined) - ONLY CLOTHING SUBCATEGORY
$sql = "SELECT p.*, v.price, v.mrp, v.weight_size as size, v.stock_qty, v.id as variant_id 
        FROM products p 
...
// Filter Mapping Logic - Handle both old (Adult/Senior) and new (Adult Dog/Senior Dog) lifestage values
if($current_stage) { 
    if($current_stage == 'Puppy') {
        $sql .= " AND p.life_stage = 'Puppy'";
    } elseif($current_stage == 'Adult Dog') {
        $sql .= " AND (p.life_stage = 'Adult Dog' OR p.life_stage = 'Adult')";
    } elseif($current_stage == 'Senior Dog') {
        $sql .= " AND (p.life_stage = 'Senior Dog' OR p.life_stage = 'Senior')";
    } else {
        $sql .= " AND p.life_stage = '$current_stage'";
    }
}
...
<style>
    ...
    .hero-glimmer { position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: linear-gradient(rgba(15,28,63,0.7), rgba(15,28,63,0.7)), url('uploads/productpageHero/dogclothing.jpg'); background-size: cover; background-position: center; animation: slowReflect 20s infinite alternate; z-index: -1; }
...
<div class="hero-vault animate__animated animate__fadeIn">
    ...
    <div class="animate__animated animate__zoomIn" style="text-align: center;">
        <h1>Dog Clothing Hub</h1>
        <p style="font-weight: 800; text-transform: uppercase; letter-spacing: 4px; color: rgba(255,255,255,0.95);">Fashion & Comfort Collection</p>
    </div>
</div>
...
<div class="catalog-viewport">
    <aside class="sidebar-filters">
        ...
        <div class="filter-group">
            ...
            <a href="dog_clothing.php" class="st-link <?php if(!$current_stage) echo 'active'; ?>">All Stages</a>
            <a href="?stage=Puppy" class="st-link <?php if($current_stage=='Puppy') echo 'active'; ?>">Puppy</a>
            <a href="?stage=Adult Dog" class="st-link <?php if($current_stage=='Adult Dog') echo 'active'; ?>">Adult Dog</a>
            <a href="?stage=Senior Dog" class="st-link <?php if($current_stage=='Senior Dog') echo 'active'; ?>">Senior Dog</a>
        </div>
    ...
    <div class="product-card <?php echo $is_out_of_stock ? 'out-of-stock' : ''; ?> animate__animated animate__fadeInUp">
        ...
        <div onclick='<?php echo !$is_out_of_stock ? "startIntelligenceLoading(" . htmlspecialchars(json_encode($p), ENT_QUOTES, "UTF-8") . ")" : ""; ?>'>
            ...
            <div class="price-zone">
                ...
                <?php if(!empty($p['size'])): ?>
                    <span style="font-size: 11px; color: var(--gray); font-weight: 700;">Size: <?php echo $p['size']; ?></span>
                <?php endif; ?>
            </div>
        </div>
        <button class="btn-action" 
                <?php echo $is_out_of_stock ? 'disabled' : ''; ?>
                onclick="<?php echo !$is_out_of_stock ? "addToCart(" . $p['variant_id'] . ", '" . addslashes($p['name']) . "', '" . $p['base_image'] . "', " . $p['price'] . ", '" . $p['size'] . "')" : 'return false;'; ?>">
        ...
        <div class="modal-console">
            ...
            <div class="console-scroll-zone">
                ...
                <div class="dash-grid">
                    ...
                    <div class="dash-item">
                        <small class="dash-label">Size Available</small>
                        <div id="qv-size" class="dash-value"><i class="fas fa-ruler-combined"></i> </div>
                    </div>
                </div>
            </div>
...
function launchQuickView(p) {
    ...
    document.getElementById('qv-size').innerHTML = `<i class="fas fa-ruler-combined" style="color:var(--primary)"></i> ${p.size || 'Standard'}`;
 ...
 */
function addToCart(variantId, name, image, price, size) {
