...
// 2. Get the sub_category ID for Toys (assuming it's named "Toys" or "Cat Toys")
$toy_cat_query = mysqli_query($conn, "SELECT id FROM sub_categories WHERE sub_name LIKE '%Toy%' OR sub_name LIKE '%toy%' LIMIT 1");
$toy_cat_row = mysqli_fetch_assoc($toy_cat_query);
$toy_cat_id = $toy_cat_row['id'] ?? 7; // Default to 7 if not found

// 3. DYNAMIC CONTENT FETCH - Get available brands for cat toys
$brand_res = mysqli_query($conn, "SELECT DISTINCT brand_name FROM products WHERE is_available = 1 AND main_cat_id = 3 AND sub_cat_id = $toy_cat_id ORDER BY brand_name ASC");
...
$sql = "SELECT p.*, v.price, v.mrp, v.weight_size, v.stock_qty, v.id as variant_id 
        ...
        WHERE p.is_available = 1 AND p.main_cat_id = 3 AND p.sub_cat_id = $toy_cat_id";
...
<style>
    ...
    .hero-glimmer { position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: linear-gradient(rgba(15,28,63,0.7), rgba(15,28,63,0.7)), url('uploads/productpageHero/cattoyherp.jpg'); background-size: cover; background-position: center; animation: slowReflect 20s infinite alternate; z-index: -1; }
...
<div class="hero-vault animate__animated animate__fadeIn">
    ...
    <div class="animate__animated animate__zoomIn" style="text-align: center;">
        <h1>Cat Toys & Playtime</h1>
        <p style="font-weight: 800; text-transform: uppercase; letter-spacing: 4px; color: rgba(255,255,255,0.95);">Interactive Fun & Entertainment</p>
    </div>
</div>
...
<div class="catalog-viewport">
    <aside class="sidebar-filters">
        ...
        <div class="filter-group">
            <h4>Life Stage</h4>
            <a href="cat_Toys.php" class="st-link <?php if(!$current_stage) echo 'active'; ?>">All Ages</a>
    ...
    <main class="product-grid">
        ...
        if(mysqli_num_rows($result) == 0) {
            echo '<div style="grid-column: 1/-1; text-align: center; padding: 60px 20px;">
                    ...
                    <h3 style="color: var(--navy); margin-bottom: 10px;">No Toys Found</h3>
...
<script>
/**
 * FURRYMART - Cat Toys Product Console
 */
...
function launchQuickView(p) {
    ...
    document.getElementById('qv-desc').innerText = p.description || 'Premium quality toy designed for your cat\'s entertainment and exercise.';
